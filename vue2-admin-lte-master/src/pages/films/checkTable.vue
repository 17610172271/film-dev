        ...
        <div class="lk-table m-t-sm">
            ...
            <ul class="table-tbody clear" v-for="(item, index) in data.rows">
                ...
                <li class="col-xs-2 p-n" v-show="selectVal.indexOf('操作')!=-1">
                    <a href="javascript:;" title="通过" class="candle-btn btn" @click="checkFilm('pass', item.id)"><i
                        class="fa fa-check"></i></a>
                    <a href="javascript:;" title="不通过" class="candle-btn btn" @click="checkFilm('notpass')"><i
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        methods: {
            getList () {
                this.loading = true
                this.$http.get(api.films.checkList, {
                    params: {
//                        offset: this.offset,
//                        limit: this.limit,
...
//                        status: this.searchOptions[8].value
            ...
            checkFilm(type, id) {
                this.$confirm('您确认将该影片通过审核吗?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.$http.get(api.films.check, {
                        params: {
//              id: id,
//              type: type
                        }
        ...
        watch: {
            ...
            page (val) {
                this.$router.replace({name: 'films_check_list', query: {type: this.$route.query.type, page: val}})
                this.getList()
            },
            limit (val) {
                this.getList()
