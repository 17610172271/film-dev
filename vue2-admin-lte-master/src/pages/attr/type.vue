...
//                        status: this.searchOptions[1].value
            ...
            delItem(id) {
                this.$confirm('此操作将删除该影片类型, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.$http.get(api.attr.typeDel, {
                        params: {
//                            id: id
                        }
                    }).then(res => {
            ...
            doRight(id, type) {
                this.$confirm('您将要提升该类型权重, 是否继续?', '提示', {
                    ...
                    this.$http.get(api.attr.typeRight, {
                        params: {
//                            id: id,
//                            type: type
                        }
            ...
            getDetailData (id) {
                this.$http.get(api.attr.typeDetail, {
                    params: {
//                        id: id
                    }
                }).then(res => {
                    if (res.data.code === 1) {
                        ...
                        this.typeDetail = JSON.stringify(res.data.data)
                    }
                })
            },
            resetData () {
            ...
            dailogReset () {
                if (this.type === 'add') {
                    ...
                    this.dailogVal = JSON.parse(this.typeDetail)
                    console.log(this.dailogVal)
                }
            },
            dailogSubmit () {
                ...
                if (this.dailogVal.name && this.dailogVal.rename && this.dailogVal.desc && this.dailogVal.status) {
                    if (this.type === 'add') {
                        this.$http.post(api.attr.typeAdd, this.dailogVal).then(res => {
                            if (res.data.code === 1) {
                                this.$message({
                                    type: 'success',
                                    message: '类型添加成功'
                        ...
                        this.$http.post(api.attr.typeEdit, this.dailogVal).then(res => {
                            if (res.data.code === 1) {
                                this.$message({
                                    type: 'success',
                                    message: '类型编辑成功'
        ...
        watch: {
            ...
            page (val) {
                this.$router.replace({name: 'attr_type', query: {page: val}})
                this.getList()
            },
            limit (val) {
                this.getList()
